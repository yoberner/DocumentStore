...
public class TrieImpl<Value> implements Trie<Value> {
    ...
    private Node<Value> put(Node<Value> node, String key, Value val, int d) {
        ...
        while (st.hasMoreTokens()) {
            ...
            if (d == key2.length()) { //we've reached the last node in the key, set the value for the key and return the node
                if (node.valList == null) {
                    node.valList = new ArrayList<Value>();
                }
                if (node.valList.contains(val)) { //checks for duplicates
                    continue;
                }
                node.valList.add(val);
                d = 0;
                continue;
            }
    ...
    private void collect(Node<Value> node, StringBuilder prefix, List<Value> results) {
        if (node.valList != null && !node.valList.isEmpty()) { //if this node has a value, add it’s key to the list
            LinkedHashSet<Value> hashset = new LinkedHashSet<Value>(node.valList);
            List<Value> listWithNoDuplicates = new ArrayList<Value>(hashset);
            results.addAll(listWithNoDuplicates);
        }
    ...
    private void deleteAndCollect(Node<Value> node, StringBuilder prefix, Set<Value> results) {
        if (node.valList != null && !node.valList.isEmpty()) { //if this node has a value, add it’s key to the list
            results.addAll(node.valList);
            node.valList = null; //delete
        }
        for (char c = 0; c < this.size; c++) { //visit each non-null child/link
            if (node.links[c] != null) {
                prefix.append(c); //add child's char to the string
    ...
    private Node<Value> deleteAll(Node<Value> node, String key, int d, Set<Value> results) {
        ...
        if (node.valList != null && !node.valList.isEmpty()) { //this node has a val – do nothing, return the node
            return node;
        }	
    ...
    private Node<Value> delete(Node<Value> node, String key, int d, Value val) {
        ...
        if (d == key.length()) { //we're at the node to delete - set the val to null
            if (node.valList != null && !node.valList.isEmpty()) {
                if (node.valList.contains(val)) {
                    this.valToDelete = val;
                    node.valList.remove(val);
                }
            }   
        }
        else { //continue down the trie to the target node
