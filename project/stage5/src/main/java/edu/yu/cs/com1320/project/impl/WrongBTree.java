 ...
 */
public class WrongBTree<Key extends Comparable<Key>, Value>
{
    ...
    private static final class Node
    {
        ...
        private Entry[] entries = new Entry[WrongBTree.MAX]; // the array of children
        private Node next;
    ...
    private Node put(Node currentNode, Key key, Value val, int height)
    {
        ...
        if (currentNode.entryCount < WrongBTree.MAX)
        {
    ...
    {
        Node newNode = new Node(WrongBTree.MAX / 2);
        //by changing currentNode.entryCount, we will treat any value
        //at index higher than the new currentNode.entryCount as if
        //it doesn't exist
        currentNode.entryCount = WrongBTree.MAX / 2;
        //copy top half of h into t
        for (int j = 0; j < WrongBTree.MAX / 2; j++)
        {
            newNode.entries[j] = currentNode.entries[WrongBTree.MAX / 2 + j];
        }
        //external node
        if (height == 0)
        {
